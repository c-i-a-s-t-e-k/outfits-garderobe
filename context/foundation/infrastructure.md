...
- **The PR environment's public domain answers 400.** `ALLOWED_HOSTS` is copied from `production`, so Django rejects the `*-pr-<N>.up.railway.app` host on every path, `/health/` included. Railway's own healthcheck still passes because it uses the always-allowed `healthcheck.railway.app` host. A green PR check therefore proves build, `migrate` and healthcheck, not a clickable preview. Browser smoke checks happen on production after merge, unless the PR environment gets its own `ALLOWED_HOSTS`/`CSRF_TRUSTED_ORIGINS` (e.g. from `${{RAILWAY_PUBLIC_DOMAIN}}`). First seen on PR #4, 2026-09-13.
