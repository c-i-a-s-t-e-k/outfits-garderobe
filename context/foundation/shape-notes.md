...
checkpoint:
  ...
  gray_areas_resolved:
    - topic: "pain category"
      decision: "workflow friction — tools exist for cataloging but not for outfit composition recall"
    - topic: "insight vs status quo"
      decision: "existing wardrobe apps focus on cataloging clothes, not composing and recalling outfits"
    - topic: "primary persona scope"
      decision: "style-conscious hobbyists who actively plan outfits but are not professional stylists"
    - topic: "auth strategy"
      decision: "login with account (email + password / OAuth); flat user model — no roles"
    - topic: "MVP scope"
      decision: "cut plugin architecture to v2; keep visual outfit UI, outfit photos, tags (replaced collections) in v1"
    - topic: "collections vs tags"
      decision: "replaced collections with tags on outfits — simpler model, same filtering capability"
    - topic: "clothing composition field"
      decision: "removed 'skład' field from clothing — photo, type, and description are sufficient"
    - topic: "clothing deletion cascade"
      decision: "deleting a clothing item removes it from outfits; outfit stays but shows incomplete state with quick fix/delete actions"
    - topic: "timeline"
      decision: "4–5 weeks after-hours; user acknowledged sustained-effort cost"
  frs_drafted: 10
...
Acknowledged on 2026-05-26: 4–5-week MVP requires sustained dedication; user accepted. Scope cut: modular plugin architecture moved to v2.

## MVP flow
1. Rejestracja / login
2. Dodanie ubrań (zdjęcie + typ + opis)
3. Złożenie outfitu z dodanych ubrań (wizualny interfejs)
4. Dodanie własnego zdjęcia w danym stroju
5. Przeglądanie garderoby przez pryzmat zdjęć outfitowych
6. Tagowanie outfitów (np. "letnie", "smart-casual") i filtrowanie po tagach

Wycięte z v1: modularna architektura (system pluginów) → v2
Zmienione: kolekcje → tagi (prostszy model); pole "skład" usunięte z ubrania
...
All 5 greenfield quality elements present (+ preserved behavior n/a for greenfield). Status: accepted. No gaps.
