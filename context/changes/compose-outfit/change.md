...
- **Manual checks 4.9–4.11 (developer, 2026-09-13):** passed on production — compose works end to end and the outfit is first in the grid; a second account sees an empty wardrobe, gets 404 on the first account's detail URL and sees none of its garments in the picker; wardrobe and compose at 360 px show two tiles per row with no horizontal scroll.
- **Finding (production, OG-9):** clicking *Save* several times while a slow submit is in flight creates duplicates — in both the add-garment form and the compose form (one production account got two identical outfits, `outfit-1` and `outfit-2`, from a single form). This is not the "duplicates allowed" decision in the plan (a deliberately composed identical garment set); it is a double submit. Out of scope for S-03; tracked as Jira OG-9 (To Do) for a separate change.
