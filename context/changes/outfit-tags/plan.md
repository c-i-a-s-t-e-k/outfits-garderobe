...
- [x] 4.1 The full suite passes on the rebased branch: `uv run pytest` — e888717
- [x] 4.2 Nothing is left unmigrated after the rebase: `uv run python manage.py makemigrations --check --dry-run` — e888717
- [x] 4.3 The deploy-configuration guard still passes: `uv run pytest accounts/tests/test_deploy_config.py` — e888717
- [x] 4.4 The PR against `master` exists and contains only this slice's commits: `gh pr view feat/outfit-tags --json baseRefName,commits` — e888717
- [x] 4.5 The PR has no merge conflicts: `gh pr view feat/outfit-tags --json mergeable` — e888717
- [x] 4.6 The Railway PR environment build (the PR status check) is green — e888717
